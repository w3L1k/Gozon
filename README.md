...
1. Создать заказ с небольшой суммой (например, 150)
2. Статус заказа через несколько секунд станет `Finished` / `Paid`

### Неуспешный платёж

1. Создать заказ с суммой, превышающей баланс (например, 999999)
2. Статус заказа станет `Cancelled` / `Failed`
